...
GitHub / Wakapi                  packages/sources - one adapter per source
 ...
 resolveSiteView()               packages/core - content + source data -> SiteView
        |
        v
   read API   ->   Nuxt SSR       apps/server/src/routes/read.ts -> apps/web (per request)
```
...
| `@lg/web` | Nuxt 3 SSR: the public site, the `/admin` CMS, the on-site `/docs` renderer, and `/md/<slug>` pages for Markdown assets. |
...
heatmap buckets pre-computed. The SSR page (`apps/web/src/pages/index.vue`)
fetches that per request through `apps/web/src/server/utils/site.ts`, which keeps a
...
Nuxt renders the page server-side and loads the `SiteView` per request. Areas are
routes, so switching one is a navigation rather than a tab: `SiteChrome` carries the
nav and the theme toggle, `SitePanels` renders the area's modules, and motion is
disabled under `prefers-reduced-motion`. `components/shell/Module.vue` maps each
module `kind` to a component, so adding a kind is an entry in that map plus one in
`PANEL_FOR_KIND`, and nothing else on the render path. The CMS at `/admin` is a
client-side app on its own route. The `/docs`
