#!/usr/bin/env node
/**
 * Run this AFTER finishing manual/dashboard work started with
 * scripts/dev-up.mjs. Stops exactly the backend + frontend processes that
 * dev-up.mjs started for this `--id` (never a broad process-name kill — only
 * the recorded PIDs), then deletes that id's database, media dir, and logs so
...
/** Kills exactly this one PID (and, on Windows, its child processes) — never a
...
function main() {
  ...
  if (fs.existsSync(sessionFile)) {
    ...
    killPid(session.webPid, "frontend", log);
  } else {
